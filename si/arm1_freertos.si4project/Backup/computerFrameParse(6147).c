...
AppSettingTypeDef hDefaultSetting =
{
    ...
    //系统工作状态
    .workmode = INS_BOOT_MODE_1,
    .datamode = INS_DATA_MODE_1,
    //GNSS臂杆参数 3
    .gnssMechanicalMigration_x = 1.0,
    .gnssMechanicalMigration_y = 1.0,
    .gnssMechanicalMigration_z = 1.0,
    //航向角补偿 4
    .courseAngleCompensation = 1.0,
    //用户设置的坐标轴类型 5
    .imuAxis = INS_DATA_MODE_1,
    //时间补偿 6
    .timeCompensation = 2,
    //GNSS基线长度 7
    .gnssBaselineLength = 0.8,
...
void frameParse(uint8_t* pData, uint16_t len)
{
    ...
    if(valid)
    {
        ...
        case CMD_SET_OUTPUT_FORMAT:				//串口输出模式
            ...
            id = hSetting.serialFrameSetting[0].baudrate;
            ...
            id = hSetting.serialFrameSetting[1].baudrate;
...
#define PARA_STORE_ADDRESS		((uint32_t)0x08104000)
...
{
    fmc_read_8bit_data(PARA_DEFAULT_ADDRESS, sizeof(AppSettingTypeDef), (int8_t*)&hDefaultSetting.saved);
    if(1 != hDefaultSetting.saved)
    {
