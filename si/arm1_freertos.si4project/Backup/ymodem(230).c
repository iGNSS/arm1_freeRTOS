...
//    enum rym_code code;
    ...
    for (i = 0; i < tm_sec; i++)
    {
        ...
        len = _rym_read_data(ctx, _RYM_SOH_PKG_SZ);
        if (len == (_RYM_SOH_PKG_SZ))
            break;
    }
    if (i == tm_sec)
        return -RYM_ERR_TMO;
...
static ins_err_t _rym_trans_data(
        ...
        enum rym_code *code)
{
    const ins_size_t tsz = 1+2+data_sz+2;
    uint16_t recv_crc;

    /* seq + data + crc */
    ins_size_t i = _rym_read_data(ctx, tsz);
    if(ctx->buf[0] == RYM_CODE_EOT)return INS_EOK;
    if (i != tsz)
        return -RYM_ERR_DSZ;
