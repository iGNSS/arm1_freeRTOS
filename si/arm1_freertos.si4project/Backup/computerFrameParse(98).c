...
void frameParse(uint8_t* pData, uint16_t len)
{
    ...
    if(valid)
    {
        ...
        case CMD_SET_OUTPUT_FORMAT:				//串口输出模式
            ...
            Uart_TxInit(UART_TXPORT_COMPLEX_8, comm_baud_table[id].index, UART_PARITY_NONE, UART_STOPBIT_ONE, UART_RS422, UART_ENABLE);
            Uart_RxInit(UART_RXPORT_COMPLEX_8, comm_baud_table[id].index, UART_PARITY_NONE, UART_STOPBIT_ONE, UART_RS422, UART_ENABLE);
