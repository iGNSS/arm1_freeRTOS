...
char navi_test_str[200] = {0,};
uint8_t navi_test_sendBuf[200];
void navi_test_pack(void)
{
    uint16_t len;
    navi_test_info.accelX = imu_info.accelX_l;
    navi_test_info.accelX |= imu_info.accelX_h;
    navi_test_info.accelY = imu_info.accelY_l;
    navi_test_info.accelY |= imu_info.accelY_h;
    navi_test_info.accelZ = imu_info.accelZ_l;
    navi_test_info.accelZ |= imu_info.accelZ_h;
    navi_test_info.gyroX  = imu_info.gyroX_l;
    navi_test_info.gyroX  |= imu_info.gyroX_h;
    navi_test_info.gyroY  = imu_info.gyroY_l;
    navi_test_info.gyroY  |= imu_info.gyroY_h;
    navi_test_info.gyroZ  = imu_info.gyroZ_l;
    navi_test_info.gyroZ  |= imu_info.gyroZ_h;
    navi_test_info.sensor_temp = imuParseData.sensorTemp * 32768 / 200;
    sprintf(navi_test_str, "%d,%0.3f,%x,%x,%x,%x,%x,%x,%0.3f,%d,%x,%x,,%d,,%d,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,", \
            navi_test_info.counter, navi_test_info.timestamp, navi_test_info.accelX, navi_test_info.accelY, navi_test_info.accelZ, \
    ...
    len = strlen(navi_test_str) + 6;

    navi_test_sendBuf[0] = 0xFA;
    navi_test_sendBuf[1] = 0x55;
    navi_test_sendBuf[2] = 0xAA;
    navi_test_sendBuf[3] = 0xEE;
    memcpy((void*)&navi_test_sendBuf[4], (void*)navi_test_str, strlen(navi_test_str));
    navi_test_sendBuf[len-2] = 0x00;
    navi_test_sendBuf[len-1] = 0xFF;
    Uart_SendMsg(UART_TXPORT_COMPLEX_8, 0, len, navi_test_sendBuf);
    if(navi_test_info.gps_valid)
