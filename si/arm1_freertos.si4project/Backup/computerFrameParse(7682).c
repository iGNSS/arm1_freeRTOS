...
AppSettingTypeDef hDefaultSetting =
{
    ...
    {'A', 'R', 'M', '1', '_', 'V', 'E', 'R', ':', ' ', '1', '.', '0', '.', '0', '\r', '\n'},
    {'A', 'R', 'M', '2', '_', 'V', 'E', 'R', ':', ' ', '1', '.', '0', '.', '0', '\r', '\n'},
...
void navi_test_switch(uint8_t* pData)
{
    if(0 == strncmp("NAVI_TEST_ON", (char const *)pData, strlen("NAVI_TEST_ON")))
    {
        naviTestFlg = 1;
    }
    else if(0 == strncmp("NAVI_TEST_OFF", (char const *)pData, strlen("NAVI_TEST_OFF")))
    {
        naviTestFlg = 0;
    }
    else if(0 == strncmp("BOOT_START", (char const *)pData, strlen("BOOT_START")))
    {
        bootFlg = 1;
        ymodem_give_sem();
    }
    ...
    if(valid)
    {
        ...
        switch(tCmd)
        {
            case CMD_SET_OUTPUT_FORMAT:				//串口输出模式
                ...
                vTaskDelay_ms(20);
#if (configUse_COMM == COMM_MODE_RS422)
                baud = hSetting.settingData.serialFrameSetting[0].baudrate;
                Uart_TxInit(UART_TXPORT_COMPLEX_8, (EUartBaudrate)baud, UART_PARITY_NONE, UART_STOPBIT_ONE, UART_RS422, UART_ENABLE);
                Uart_RxInit(UART_RXPORT_COMPLEX_8, (EUartBaudrate)baud, UART_PARITY_NONE, UART_STOPBIT_ONE, UART_RS422, UART_ENABLE);
#elif (configUse_COMM == COMM_MODE_RS232)
                baud = hSetting.settingData.serialFrameSetting[1].baudrate;
                gd32_usart_set_baud(comm_baud_table[baud].baudrate);
#endif
                //comm_set_customPara();
                //vTaskDelay_ms(20);
                //NVIC_SystemReset();
            ...
            case CMD_SET_OUTPUT_PORT:					//设定输出端口
                if((*(pDat + 1)) == FRAME_END)
                {
                    hSetting.outputPort = *pDat;
                }
...
{
    return hSetting.settingData.serialFrameSetting[channel].freq;
}

uint8_t comm_read_currentFrameType(uint8_t channel)
{
    return hSetting.settingData.serialFrameSetting[channel].frameType;
}

uint8_t comm_read_dataMode(void)
{
    return hSetting.settingData.datamode;
}
    ...
    for( ;; )
    {
        if(pdTRUE == xQueueReceive( xCommQueue, &status, pdMS_TO_TICKS(100)))//portMAX_DELAY
        {
            ...
            else if(1 == status)//串口发送至上位机
            {
                ...
                {
                    freq = comm_read_currentFreq(channel);
                    reportCount = TIM_BASE_PERIOD / freq;

                    if(0 == (freqCount % reportCount))
                    {
                        protocol_report(channel);
                    }
                }

                if(freqCount >= TIM_BASE_PERIOD)freqCount = 0;
            }
        }
...
{
    uint8_t  frame[100];
    uint16_t len = sizeof(hSetting.ARM1_FW_Ver) * 3 + 6;

    frame[0] = 0xFA;
    frame[1] = 0x55;
    frame[2] = cmd >> 8;
    frame[3] = cmd;
    memcpy((void*)&frame[4], (void*)&hSetting.ARM1_FW_Ver, sizeof(hSetting.ARM1_FW_Ver) * 3);
    frame[len - 2] = 0x00;
    frame[len - 1] = 0xFF;
...
#elif (configUse_COMM == COMM_MODE_RS422)
    baud = hSetting.settingData.serialFrameSetting[0].baudrate;
    Uart_TxInit(UART_TXPORT_COMPLEX_8, (EUartBaudrate)baud, UART_PARITY_NONE, UART_STOPBIT_ONE, UART_RS422, UART_ENABLE);
    Uart_RxInit(UART_RXPORT_COMPLEX_8, (EUartBaudrate)baud, UART_PARITY_NONE, UART_STOPBIT_ONE, UART_RS422, UART_ENABLE);
#elif (configUse_COMM == COMM_MODE_RS232)
