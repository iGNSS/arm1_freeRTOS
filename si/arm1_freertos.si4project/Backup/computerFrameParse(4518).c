...
{
	fmc_erase_sector_by_address(PARA_STORE_ADDRESS);
...
void comm_para_ehco_rsp(uint16_t cmd)
{
    ...
    frameEnd[2] = cmd >> 8;
    frameEnd[3] = cmd;
    memcpy((void*)&frameEnd[4], (void*)&hSetting.settingData.serialFrameSetting[0].frameType, sizeof(SettingDataTypeDef));
    frameEnd[len-2] = 0x00;
    frameEnd[len-1] = 0xFF;
    
#if (configUse_COMM == COMM_MODE_RS422)
    Uart_SendMsg(UART_TXPORT_COMPLEX_8, 0, len, frameEnd);
#elif (configUse_COMM == COMM_MODE_RS232)
